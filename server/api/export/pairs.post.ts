...
export default defineEventHandler(async (event) => {
  ...
  const outPath = join(tmpdir(), `export_pairs_${Date.now()}.jsonl`)
  const args = ['-m', 'bandofy_export_studio', 'export-pairs-jsonl', '--out', outPath]
  ...
  try {
    ...
    setHeader(event, 'Content-Disposition', 'attachment; filename="pairs.jsonl"')
    return data
  } finally {
